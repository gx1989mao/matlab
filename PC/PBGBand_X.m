function PBGBand_X(ea,eb,R,PCType,Keach,TEorTM)
...
NEIG=2; %NEIG: The number cut of the Eigvalue.
...
%    disp(strcat('---K vector No.',num2str(knum),'---',num2str(NumberK))) 
   ...
   if (TEorTM==1)
       for ii=1:1:NPW
           for jj = 1:1:NPW
               KGmn_mat(ii,jj) = sqrt((kx-Gx_m(ii,jj))^2+(ky-Gy_m(ii,jj))^2)*sqrt((kx-Gx_n(ii,jj))^2+(ky-Gy_n(ii,jj))^2);
           end
       end
%        KGmn_mat=norm((kx-Gx_m),(ky-Gy_m))*norm((kx-Gx_n),(ky-Gy_n));
       H=KGmn_mat.*ek_mat;
       %TM part
...
figure;
clf;
eigval2=zeros(2,Keach+1);
eigval3=zeros(2,Keach+1);
x3=zeros(1,Keach+1);
for nn = 1:1:Keach+1
    eigval2(:,nn) = eigval(:,Keach+1+nn);
    eigval3(:,nn) = eigval(:,nn);
    x3(1,nn) = x(1,nn);
end

h=plot(x3,eigval3,'b-',x3,eigval2,'b-',x3,0.5.*sqrt((4.*x3).^2.+0.25),'r');
% yyy = 0.5/pi*sqrt(((2*pi*x3)^2+(0.5*pi)^2)/1);
...
if (PCType==1)
   ...
   text(x(1)-0.005,-0.03, '\Gamma','FontSize',14)
   text(x1-0.005,-0.03, 'X','FontSize',14)
...
if (TEorTM==0)
    save datate.mat ea eb R PCType Keach eigval eigval2 eigval3 x;
end
if (TEorTM==1)
    save datatm.mat ea eb R PCType Keach eigval eigval2 eigval3 x;
end   
